...
/**!phpjs;module:_phpjs_shared
 */
exports._phpjs_shared_bc = function _phpjs_shared_bc () {
  ...
  var libbcmath = {
     ...
     */
    bc_num: function () {
      module.n_sign = null; // sign
      module.n_len = null; /* (int) The number of digits before the decimal point. */
      module.n_scale = null; /* (int) The number of digits after the decimal point. */
      //this.n_refs = null; /* (int) The number of pointers to this number. */
      //this.n_text = null; /* ?? Linked list for available list. */
      module.n_value = null; /* array as value, where 1.23 = [1,2,3] */
      module.toString = function () {
        var r, tmp;
        tmp = module.n_value.join('');

        // add minus sign (if applicable) then add the integer part
        r = ((module.n_sign == libbcmath.PLUS) ? '' : module.n_sign) + tmp.substr(0, module.n_len);

        // if decimal places, add a . and the decimal part
        if (module.n_scale > 0) {
          r += '.' + tmp.substr(module.n_len, module.n_scale);
        }
        return r;
      };
    },
...
/* Set up the divide.  Move the decimal point on n1 by n2's scale.
   Remember, zeros on the end of num2 are wasted effort for dividing. */
    ...
    MUL_BASE_DIGITS: 80,
    MUL_SMALL_DIGITS: 20,//(this.MUL_BASE_DIGITS / 4),
