...
/**!phpjs;module:array
 */
exports.array_walk = function array_walk (array, funcname, userdata) {
  ...
  try {
    if (typeof funcname === 'function') {
      ...
      ini = module.php_js.ini['phpjs.no-eval'];
      if (ini && (
        parseInt(ini.local_value, 10) !== 0 && (!ini.local_value.toLowerCase || ini.local_value.toLowerCase() !== 'off')
      )) {
        if (arguments.length > 2) {
          for (key in array) {
            module.window[funcname](array[key], key, userdata);
          }
        }
        else {
          for (key in array) {
            module.window[funcname](array[key], key);
          }
        }
      }
      else {
